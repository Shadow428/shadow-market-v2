/* ──────────────────────────────────────────────────────────
   PaymentModal.tsx — UPI QR payment flow modal
   ────────────────────────────────────────────────────────── */
import { useState, useEffect } from 'react';
import type { Product } from '@/lib/storage';
import { addOrder } from '@/lib/storage';

interface Props {
  product: Product;
  onClose: () => void;
  onSuccess: (orderId: string) => void;
}

/* Replace with your real UPI ID */
const UPI_ID = 'shadowmarketplace@upi';
const UPI_NAME = 'Shadow Marketplace';

type Step = 'qr' | 'txid' | 'done';

export default function PaymentModal({ product, onClose, onSuccess }: Props) {
  const [step, setStep] = useState<Step>('qr');
  const [txId, setTxId] = useState('');
  const [txError, setTxError] = useState('');

  const upiUri = `upi://pay?pa=${UPI_ID}&pn=${encodeURIComponent(UPI_NAME)}&am=${product.price}&cu=INR&tn=${encodeURIComponent('Shadow Marketplace - ' + product.name)}`;
  const qrUrl  = `https://api.qrserver.com/v1/create-qr-code/?size=220x220&color=9B30FF&bgcolor=0A0A12&data=${encodeURIComponent(upiUri)}`;

  /* Close on Escape */
  useEffect(() => {
    const handler = (e: KeyboardEvent) => { if (e.key === 'Escape') onClose(); };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  }, [onClose]);

  function handlePaid() {
    if (!txId.trim()) {
      setTxError('Please enter the UPI transaction ID.');
      return;
    }
    if (txId.trim().length < 6) {
      setTxError('Transaction ID seems too short. Please check and try again.');
      return;
    }
    const order = addOrder({
      productId:   product.id,
      productName: product.name,
      amount:      product.price,
      transactionId: txId.trim(),
      status:      'Pending',
    });
    setStep('done');
    onSuccess(order.id);
  }

  return (
    <div
      className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in"
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div className="modal-panel rounded-2xl w-full max-w-md animate-scale-in relative overflow-hidden">
        {/* Scanline overlay */}
        <div className="scanline absolute inset-0 pointer-events-none z-0" />

        {/* Header */}
        <div
          className="flex items-center justify-between px-6 py-4 relative z-10"
          style={{ borderBottom: '1px solid hsl(270 40% 18%)' }}
        >
          <div>
            <h2 className="font-bold text-lg" style={{ color: 'hsl(270 100% 90%)' }}>
              Complete Payment
            </h2>
            <p className="text-xs mt-0.5" style={{ color: 'hsl(270 30% 55%)' }}>
              {product.name}
            </p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full flex items-center justify-center transition-colors"
            style={{ color: 'hsl(270 30% 55%)', background: 'hsl(270 20% 12%)' }}
            aria-label="Close"
          >
            ✕
          </button>
        </div>

        {/* Body */}
        <div className="px-6 py-5 relative z-10">

          {/* ── Step: QR ────────────────────────────────── */}
          {step === 'qr' && (
            <div className="flex flex-col items-center gap-5">
              {/* Amount */}
              <div className="text-center">
                <p className="text-xs uppercase tracking-widest mb-1" style={{ color: 'hsl(270 30% 55%)' }}>
                  Amount to Pay
                </p>
                <div
                  className="text-4xl font-black glow-text"
                  style={{ color: 'hsl(270 100% 70%)' }}
                >
                  ₹{product.price}
                </div>
              </div>

              {/* QR Code */}
              <div
                className="p-3 rounded-xl animate-pulse-glow"
                style={{
                  background: 'hsl(240 20% 5%)',
                  border: '1px solid hsl(270 60% 35%)',
                }}
              >
                <img
                  src={qrUrl}
                  alt="UPI QR Code"
                  width={220}
                  height={220}
                  className="rounded-lg block"
                />
              </div>

              {/* UPI info */}
              <div
                className="w-full text-center text-sm rounded-lg py-2 px-4"
                style={{
                  background: 'hsl(270 100% 60% / 0.08)',
                  border: '1px solid hsl(270 100% 60% / 0.2)',
                  color: 'hsl(270 60% 75%)',
                }}
              >
                UPI ID: <span className="font-mono font-bold" style={{ color: 'hsl(270 100% 80%)' }}>{UPI_ID}</span>
              </div>

              <p className="text-xs text-center" style={{ color: 'hsl(270 30% 50%)' }}>
                Scan the QR code with any UPI app (PhonePe, GPay, Paytm, etc.)
              </p>

              {/* CTA */}
              <button
                onClick={() => setStep('txid')}
                className="btn-primary w-full py-3 rounded-xl text-base"
              >
                I've Paid →
              </button>
            </div>
          )}

          {/* ── Step: Transaction ID ─────────────────────── */}
          {step === 'txid' && (
            <div className="flex flex-col gap-5">
              <div className="text-center">
                <div className="text-3xl mb-2">🔐</div>
                <h3 className="font-bold text-base mb-1" style={{ color: 'hsl(270 100% 90%)' }}>
                  Enter Transaction ID
                </h3>
                <p className="text-xs" style={{ color: 'hsl(270 30% 55%)' }}>
                  Find the UPI reference / transaction ID in your payment app
                </p>
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'hsl(270 60% 65%)' }}>
                  UPI Transaction ID
                </label>
                <input
                  type="text"
                  value={txId}
                  onChange={(e) => { setTxId(e.target.value); setTxError(''); }}
                  placeholder="e.g. 123456789012"
                  className="input-field w-full px-4 py-3 rounded-lg font-mono text-sm"
                  onKeyDown={(e) => { if (e.key === 'Enter') handlePaid(); }}
                  autoFocus
                />
                {txError && (
                  <p className="text-xs" style={{ color: 'hsl(0 84% 65%)' }}>{txError}</p>
                )}
              </div>

              <div className="flex gap-3">
                <button
                  onClick={() => setStep('qr')}
                  className="btn-secondary flex-1 py-3 rounded-xl text-sm"
                >
                  ← Back
                </button>
                <button
                  onClick={handlePaid}
                  className="btn-primary flex-1 py-3 rounded-xl text-sm"
                >
                  Submit Order
                </button>
              </div>
            </div>
          )}

          {/* ── Step: Done ──────────────────────────────── */}
          {step === 'done' && (
            <div className="flex flex-col items-center gap-5 py-4">
              <div
                className="w-16 h-16 rounded-full flex items-center justify-center text-3xl"
                style={{
                  background: 'hsl(145 60% 40% / 0.15)',
                  border: '2px solid hsl(145 60% 50%)',
                  boxShadow: '0 0 20px hsl(145 60% 40% / 0.3)',
                }}
              >
                ✓
              </div>
              <div className="text-center">
                <h3 className="font-bold text-lg mb-1" style={{ color: 'hsl(145 70% 60%)' }}>
                  Order Placed!
                </h3>
                <p className="text-sm" style={{ color: 'hsl(270 30% 60%)' }}>
                  Your order is <span className="badge-pending px-1.5 py-0.5 rounded text-xs font-bold">Pending</span> verification.
                  You will receive account details once payment is confirmed.
                </p>
              </div>
              <button
                onClick={onClose}
                className="btn-primary px-8 py-3 rounded-xl text-sm"
              >
                Close
              </button>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
